...
<b>eng2sql is a NLP tool to query a database in natural language.</b> The tool takes in input a database model and a sentence and translate the latter in a valid SQL statement able to query the input data model.
...
Please cite the paper if you use eng2sql.

#### Differences between the version of the paper <i>(fr2sql)</i> and this version <i>(eng2sql)</i>

eng2sql is not the state-of-the-art tool for copyright reasons. It's just a quick & dirty Python wrapper but it has some speed optimizations.

* [A data model is only learned from a parsing of a SQL dump file. Thus, <b>no database connection is needed</b>.](https://github.com/FerreroJeremy/eng2sql#database-input)
...
In eng2sql, Treegagger is left in favour of an import of personal configuration files (for [languages](https://github.com/FerreroJeremy/eng2sql#supported-languages), [stop words and synonyms](https://github.com/FerreroJeremy/eng2sql#thesaurus-and-stop-word-list)) to be more generic.

> Beware that eng2sql cannot therefore automatically solve the gender and number problem. So if the word <i>"student<b>s</b>"</i> is in the input sentence, it does not match with the table <i>"student"</i> in the model of data.
To do that, the equivalence <i>"student<b>s</b> → student"</i> must be appear in the [used thesaurus](https://github.com/FerreroJeremy/eng2sql#thesaurus-and-stop-word-list).
...
* [In theory, all languages can be supported.](https://github.com/FerreroJeremy/eng2sql#supported-languages)

* A grammar still parse the input sentence to generate the corresponding query structure, but now this structure is stocked in a Python class able to print [a query structure JSON file](https://github.com/FerreroJeremy/eng2sql/blob/master/README.md#json-output). Thus, the hash map for the query generation was abandoned. In addition, a multi-threading implementation was adopted.
...
To be effective eng2sql need to learn the data model of the database that the user want to query. It need to load the corresponding SQL dump file to do that.<br/>
...
<p align="center"><img src="https://raw.githubusercontent.com/FerreroJeremy/eng2sql/master/docs/database_loading.png" width="700"></p>
...
Usage:
	...
	python3 -m eng2sql.main -d <path> -l <path> -i <input-sentence> [-j <path>] [-t <path>] [-s <path>]
Parameters:
...
example of usage:
```
python3 -m eng2sql.main -d database_store/city.sql -l lang_store/english.csv -j output.json -i "Count how many city there are with the name blob?"
```

or by graphical interface by typing the following command:
```
python eng2sql/ln2sql_gui.py
```
a window like the one below will appear:
<p align="center"><img src="https://raw.githubusercontent.com/FerreroJeremy/eng2sql/master/docs/graphical_user_interface.png" width="600"></p>
...
<p align="center"><img src="https://raw.githubusercontent.com/FerreroJeremy/eng2sql/master/docs/mvc_class_diagram.png"></p>
