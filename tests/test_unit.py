...
DATABASE_PATH = os.path.join(BASE_PATH, 'eng2sql/database_store/')
LANG_PATH = os.path.join(BASE_PATH, 'eng2sql/lang_store/')
THESAURUS_PATH = os.path.join(BASE_PATH, 'eng2sql/thesaurus_store/')
...
def selectAllQueries():
    ...
    for test in selectAll:
        assert _clean_output(Eng2sql(test['database'], test['language']).get_query(test['input'])) == test['output']
...
def noTableFoundExceptionQueries():
    ...
    for test in noTable:
        with pytest.raises(Exception):
            Eng2sql(test['database'], test['language']).get_query(test['input'])
